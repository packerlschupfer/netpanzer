...
bool SelectionBoxSprite::isVisible(const iRect &world_win) const {
  if ((world_win.contains(world_pos + selection_area.min) &&
       (visible == true)) ||
...
void UnitSelectionBox::blit(Surface *surface, const iRect &world_win) {
  ...
  if (box_state == true) {
    ...
    // the box status check, because I may want the hitpoints drawn all the
    // time.
    ...
    if (GameConfig::interface_unitselectionmode ==
    ...
    {
      surface->drawBoxCorners(iRect(min_abs.x, min_abs.y, max_abs.x, max_abs.y),
                              7, selectionBoxColor);
    }
  ...
  if (GameConfig::interface_show_health || (box_state == true)) {
    ...
    int hit_bar_size =
        int((float)(max_abs.x - min_abs.x - 5) * (hitPointPercent));

    iRect r(min_abs.x + 1, max_abs.y - 5, max_abs.x - 1, max_abs.y - 1);

    surface->bltLookup(r, Palette::darkGray256.getColorArray());

    r = iRect(min_abs.x + 2, max_abs.y - 4, min_abs.x + 2 + hit_bar_size,
              max_abs.y - 3);

    surface->drawRect(r, hitBarColor);

    // Solid method.
    // surface->drawHLine(min_abs.x+1, max_abs.y-1, min_abs.x + hit_bar_size,
    // hit_bar_color); surface->drawHLine(min_abs.x+1, max_abs.y-2, min_abs.x +
    // hit_bar_size, hit_bar_color); surface->drawHLine(min_abs.x, max_abs.y-3,
    // max_abs.x+1, box_color);
  }

  if (GameConfig::interface_show_flags == true && unit_flag) {
    // unit_flag.blt( *surface, iXY( min_abs.x, min_abs.y - unit_flag.getPix().y
    // ) ); surface->bltString(min_abs.x + 2, min_abs.y - 6, "Panther1",
    // Color::white);

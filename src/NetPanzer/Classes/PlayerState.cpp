...
PlayerID NetworkPlayerState::getPlayerIndex() const { return id; }

// If you modify this array, also modify the constant above
Uint8* playerColorArray[] = {
    &Color::red, &Color::green, &Color::brown, &Color::yellow, &Color::cyan,
    &Color::gray, &Color::magenta, &Color::pink, &Color::unitAqua,
    &Color::unitYellow, &Color::unitRed, &Color::unitBlue, &Color::unitDarkBlue,
    &Color::unitLightGreen, &Color::unitGreen, &Color::unitBlueGray,
    &Color::unitDarkRed,
    //&Color::unitBlack, black removed from list
    &Color::unitDarkGreen, &Color::unitWhite, &Color::unitLightOrange,
    &Color::unitOrange, &Color::unitGray, &Color::unitDarkGray, &Color::tan,
    &Color::chartreuse, &Color::cobaltGreen, &Color::emeraldGreen,
    &Color::forestGreen, &Color::darkOliveGreen, &Color::terreVerte,
    &Color::darkBlue, &Color::darkBrown, &Color::darkCyan, &Color::darkGray,
    &Color::darkGreen, &Color::darkMagenta, &Color::darkOrange,
    &Color::darkPink, &Color::darkRed, &Color::darkYellow, &Color::lightBlue,
    &Color::lightBrown, &Color::lightCyan, &Color::lightGray,
    &Color::lightGreen, &Color::lightMagenta, &Color::lightOrange,
    &Color::lightPink, &Color::lightRed, &Color::lightYellow, &Color::gray32,
    &Color::gray64, &Color::gray96, &Color::gray128, &Color::gray160,
    &Color::gray192, &Color::gray224};
...
void PlayerState::setName(const std::string& newname) {
  ...
  for (int ac = 0; ac < (int)name.length(); ac++) {
    ...
    if (!((int)name[ac] == 34 || (int)name[ac] == 35 || (int)name[ac] == 37 ||
          (int)name[ac] == 39 || (int)name[ac] == 44 || (int)name[ac] == 94 ||
          (int)name[ac] == 96 || (int)name[ac] == 126 || (int)name[ac] == 47 ||
          (int)name[ac] == 92 || (int)name[ac] == 124 || (int)name[ac] < 32 ||
          (int)name[ac] > 126)) {
      nodbname << name[ac];
    }
  }
  ...
  do {
    ...
    for (p = 0; p < PlayerInterface::getMaxPlayers(); ++p) {
      ...
      if ((ps->isConnecting() || ps->isActive()) && ps->name == name) {
...
void PlayerState::resetAutokick() { autokick.reset(); }

bool PlayerState::checkAutokick() {
  return GameConfig::game_autokicktime > 0 &&
         autokick.checkWithTimeOut(GameConfig::game_autokicktime * 60000);
}

const std::string& PlayerState::getName() const { return name; }

void PlayerState::lockStats() { stats_locked = true; }

void PlayerState::unlockStats() { stats_locked = false; }

short PlayerState::getKills() const { return kills; }

short PlayerState::getLosses() const { return losses; }
...
short PlayerState::getObjectivesHeld() const { return objectives_held; }
...
void PlayerState::setID(PlayerID id) { this->id = id; }

FlagID PlayerState::getFlag() const { return id; }

// this "getTotal" is used to calculate the frags.
short PlayerState::getTotal() const { return kills - (losses / 2); }

void PlayerState::setPlayerStyle(unsigned char nustyle) {
  units_style = nustyle;
}

unsigned char PlayerState::getPlayerStyle() { return units_style; }

void PlayerState::setMute(bool mute) { muted = mute; }

bool PlayerState::getMute() const { return muted; }

void PlayerState::setCheat(unsigned char cheat_type) { cheating = cheat_type; }

unsigned char PlayerState::getCheat() const { return cheating; }

void PlayerState::setCheatHits(unsigned char cheat_hits) {
  cheating_hits = cheat_hits;
}

unsigned char PlayerState::getCheatHits() const { return cheating_hits; }

void PlayerState::setClientType(unsigned char cl_type) {
  client_type = cl_type;
}

unsigned char PlayerState::getClientType() const { return client_type; }

void PlayerState::setLastEncKey(unsigned char enc_key) { lastenckey = enc_key; }

unsigned char PlayerState::getLastEncKey() const { return lastenckey; }

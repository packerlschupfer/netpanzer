...
void TileSet::loadTileSet(const char *file_path) {
  ...
  try {
    ...
    tile_buffer_size =
        (tile_set_info.x_pix * tile_set_info.y_pix) * tile_set_info.tile_count;
    ...
    throw Exception("Couldn't load tileset '%s': %s", file_path, e.what());
  }
}
...
void TileSet::loadTileSet(const char *file_path, WadMapTable &mapping_table) {
  try {
    ...
    while (tile_index < tile_count) {
      ...
      while ((tile_index < tile_count) &&
             (mapping_table[tile_index].is_used == false)) {
        unused_tile_count++;
        tile_index++;
      }
      ...
      if (tile_index < tile_count) {
        ...
        while ((tile_index < tile_count) &&
               (mapping_table[tile_index].is_used == true)) {
          used_tile_count++;
          tile_index++;
        }

        if (used_tile_count != 0) {
          file->read((tile_data + (mapped_index * tile_size)), tile_size,
                     used_tile_count);
...
bool TileSet::startPartitionTileSetLoad(const char *file_path,
                                        WadMapTable &mapping_table,
  ...
  for (tile_index = 0; tile_index < tile_count; tile_index++) {
    if (mapping_table[tile_index].is_used == true) {
      partition_load_fhandle->read((tile_info + mapped_index),
                                   sizeof(TILE_HEADER), 1);
      mapped_index++;
    } else {
      partition_load_fhandle->seek(partition_load_fhandle->tell() +
                                   sizeof(TILE_HEADER));
    }
  }
...
bool TileSet::partitionTileSetLoad(WadMapTable &mapping_table,
  ...
  *percent_complete = -1;
  ...
  while ((partition_load_tile_index < tile_count) &&
    ...
    if (mapping_table[partition_load_tile_index].is_used == true) {
      partition_load_fhandle->read(
          (tile_data + (partition_load_mapped_index * tile_size)), tile_size,
          1);

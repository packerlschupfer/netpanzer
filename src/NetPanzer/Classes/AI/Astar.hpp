...
class PathRequest {
 public:
  ...
  void set(UnitID &unit_id, iXY &start, iXY &goal, unsigned short path_type,
           PathList *path, unsigned short request_type) {
...
class AstarNode {
 public:
  ...
  bool operator<(const AstarNode &other) const { return other.f < f; }
};

...
bool SelectionList::selectUnit(iXY point) {
  ...
  UnitInterface::queryUnitsAt(
      unit_list, point, PlayerInterface::getLocalPlayerIndex(), _search_player);

  select();
  if (unit_list.size() == 0) return false;
...
bool SelectionList::selectBounded(iRect bounds, bool addunits) {
  ...
  if (!tempunits.size()) return false;

...
void ServerMessageRouter::routePacket(const NetPacket* packet) {
  ...
  switch (message->message_class) {
    ...
    default:
      ...
      sprintf(string_warning_k, "Player '%s' is cheating (trying to use packet
      sniffer/editor tools)!",player->getName().c_str()); LOGGER.info("CHEAT:
      %s", string_warning_k); ChatInterface::serversay(string_warning_k);
      */
      // player->setCheat(1);

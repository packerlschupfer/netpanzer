...
void ClientConnectDaemon::shutdownConnectDaemon() {
  ...
  client_disconnect.setPlayerID(PlayerInterface::getLocalPlayerIndex());
...
void ClientConnectDaemon::startConnectionProcess() {
  ...
  LoadingView::append("");
  LoadingView::append(NPString("Connecting to server ") +
                      gameconfig->serverConnect.c_str());
}

unsigned char ClientConnectDaemon::netMessageLinkAck(
  ...
  switch (join_request_ack_mesg->getResultCode()) {
    ...
    case _join_request_result_invalid_protocol:
      ...
      LoadingView::append(getNetpanzerProtocolMessage(
          join_request_ack_mesg->getServerProtocolVersion()));
    ...
    case _join_request_result_already_connected:
      ...
      LoadingView::append(
          "The server limits multiple connections from same IP");
      rval = _connect_state_connect_failure;
      failure_display_timer.reset();
      break;
...
void ClientConnectDaemon::netMessageConnectProcessUpdate(
    const NetMessage *message) {
...
void ClientConnectDaemon::netMessageConnectProcessMessage(
    const NetMessage *message) {
  ...
  switch (state_mesg->getMessageEnum()) {
    ...
    case _connect_state_sync_complete: {
      ...
      if (NetworkState::status == _network_state_bot) {
        ...
        // it would be good to let the server provide a flag instead since it
        // knows which flags are taken...
        Surface game_flags;
...
void ClientConnectDaemon::netMessageConnectServerDisconnect(
    const NetMessage *) {
...
void ClientConnectDaemon::connectFsm(const NetMessage *message) {
  ...
  switch (connection_state) {
      ...
      if (message_id == _net_message_id_connect_join_game_request_ack) {
        ...
        if (time_out_timer.count()) {
          if (time_out_counter < _CLIENT_CONNECT_RETRY_LIMIT) {
            ...
            LoadingView::append(
                "it might be older netpanzer or server is down");
            connection_state = _connect_state_connect_failure;
            failure_display_timer.reset();
          }
        }
    ...
    case _connect_state_wait_for_server_game_setup:
      if (message &&
          message->message_id == _net_message_id_connect_server_game_setup) {
        ...
        if (result_code == _mapload_result_no_map_file) {
          ...
          CLIENT->sendMessage(&client_game_setup_ping,
                              sizeof(ConnectMesgClientGameSetupPing));
    ...
    case _connect_state_setup_client_game: {
      ...
      if (GameManager::clientGameSetup(&percent_complete) == false) {
        ...
        CLIENT->sendMessage(&client_game_setup_ack,
                            sizeof(ConnectMesgClientGameSetupAck));
...
void ClientConnectDaemon::connectProcess() { connectFsm(0); }

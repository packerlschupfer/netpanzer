...
void NetworkServer::openSession() {
  // nothing
}

void NetworkServer::hostSession() {
  if (socket) {
    socket->destroy();
    socket = 0;
  }
  ...
  try {
    Address addr = Address::resolve(*GameConfig::server_bindaddress,
                                    GameConfig::server_port, true,
                                    true);  // tcp for binding
...
void NetworkServer::closeSession() {
  if (socket) socket->destroy();

  socket = 0;
}

void NetworkServer::broadcastMessage(const NetMessage *message, size_t size) {
  ClientList::iterator i = client_list.begin();
  while (i != client_list.end()) {
    if (!(*i)->wannadie) {
      try {
...
void NetworkServer::sendMessage(const PlayerID player_index,
  ...
  LOGGER.warning("NetworkServer: sendMessage to unknown client: (index) %d",
                 player_index);
}

bool NetworkServer::getPacket(NetPacket *packet) {
...
void NetworkServer::dropClient(ClientSocket *client) {
  ClientList::iterator i = client_list.begin();
...
void NetworkServer::niceDisconnect(ClientSocket *client) {
  ClientList::iterator i = client_list.begin();
  while (i != client_list.end() && (*i)->client_socket != client) {
    ++i;
  }
...
void NetworkServer::kickClient(ClientSocket *client) {
  ClientList::iterator i = client_list.begin();
  while (i != client_list.end() && (*i)->client_socket != client) {
    ++i;
  }
...
void NetworkServer::sendRemaining() {
  ClientList::iterator i = client_list.begin();
  while (i != client_list.end()) {
    if (!(*i)->wannadie) {
...
bool NetworkServer::isAlreadyConnected(ClientSocket *client) {
...
std::string NetworkServer::getIP(const PlayerID player_index) {
  ClientList::iterator i = client_list.begin();
  while (i != client_list.end()) {
    if (!(*i)->wannadie &&
        (*i)->client_socket->getPlayerIndex() == player_index) {
...
void NetworkServer::onSocketError(TCPListenSocket *so) {
  (void)so;
  LOGGER.warning(
      "NetworkServer: Listen Socket error, something bad could happen from "
      "now");
}

TCPSocketObserver *NetworkServer::onNewConnection(TCPListenSocket *so,
...
void NetworkServer::onClientConnected(ClientSocket *s) {
...
void NetworkServer::onClientDisconnected(ClientSocket *s, const char *msg) {
  ...
  while (i != client_list.end()) {
    if ((*i)->client_socket == s) {
      LOGGER.debug(
          "NetworkServer:onClientDisconnected found client in list, preparing "
          "to die [%d]",
          player_index);
      (*i)->wannadie = true;
      break;
...
ClientSocket *NetworkServer::getClientSocketByPlayerIndex(
    const PlayerID index) {
  ClientList::iterator i = client_list.begin();
  while (i != client_list.end()) {
    if (!(*i)->wannadie && (*i)->client_socket->getPlayerIndex() == index) {

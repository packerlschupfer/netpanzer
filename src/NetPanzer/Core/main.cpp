...
BaseGameManager* initialise(int argc, char** argv) {
  ...
  option<std::string, true, false> connect_option(
      'c', "connect", "Connect to the specified netpanzer server", "");
  commandline.add(&connect_option);
  bool_option dedicated_option('d', "dedicated", "Run as dedicated server",
                               false);
  commandline.add(&dedicated_option);
  option<std::string, true, false> bot_option(
      'b', "bot",
      "Connect as bot to specific server (dedicated server accepts only "
      "localhost)",
      "");
  commandline.add(&bot_option);

  bool_option need_password(
      0, "password",
      "Ask for password when connecting, only useful on quick connect", false);
  ...
  option<std::string, true, false> master_server_option(
      '\0', "master_server",
  ...
  option<std::string, true, false> pidfile_option(
      0, "pidfile", "Create a pidfile at the specified location", "");
...
int main(int argc, char *argv[]) {
  ...
  // ScriptManager::runStr("LuaInitialize","print('Lua is working just
  // fine');");

...
DedicatedGameManager::~DedicatedGameManager() {
  ...
  if (infosocket) delete infosocket;
  Console::shutdown();
  ...
  while (!commandqueue.empty()) {
    ...
    switch (command.type) {
      ...
      case ServerCommand::STATUS: {
        //*Console::server
        std::cout << "Server " << *GameConfig::server_name << " version "
                  << Package::getVersion() << " port "
        ...
        for (i = 0; i < PlayerInterface::getMaxPlayers(); ++i) {
          ...
          if (playerstate->isActive()) {
            ...
            std::cout << std::setw(3) << static_cast<int>(i) << " "
                      ...
                      << std::setw(21) << SERVER->getIP(playerstate->getID())
                      << "\n";
          }
        }
      ...
      case ServerCommand::MAPCHANGE:
        if (!MapsManager::existsMap(command.argument)) {
          std::cout << "map '" << command.argument << "' doesn't exist."
                    << std::endl;
          break;
        }
      ...
      case ServerCommand::KICK:
        ...
        SERVER->kickClient(
            SERVER->getClientSocketByPlayerIndex((unsigned short)id));
        break;
    }
    commandqueue.pop();
  }
...
bool DedicatedGameManager::mainLoop() {
  if (heartbeat) heartbeat->checkHeartbeat();
    ...
    for (unsigned long i = 0; i < max_players; i++) {
      ...
      if (player->isActive()) {
        if (player->getCheat() == 0 && player->getCheatHits() > 1) {
          ...
          sprintf(chat_cheat_warning,
                  "Player '%s' is using unfair network tools!",
          ...
          // short) i)); // kicking is now at socket level
        }
        ...
        if (LOGGER.getLogLevel() < 7)  // if it's not DEBUG mode
        {
          ...
          SERVER->sendMessage(player->getID(), &ckmsg,
                              sizeof(SystemEnckeychange));
        } else {
        ...
        if ( player->checkAutokick() )
        {
            ...
            sprintf(chat_string, "Server kicked '%s' due to
        inactivity",player->getName().c_str()); LOGGER.info("DED: %s",
        chat_string); ChatInterface::serversay(chat_string);
            SERVER->kickClient(SERVER->getClientSocketByPlayerIndex((unsigned
        short) i));
        }
        */
        ...
        if (player->getTotal() < GameConfig::game_lowscorelimit + 1) {
          ...
          SERVER->kickClient(
              SERVER->getClientSocketByPlayerIndex((unsigned short)i));
        }
      }
    }
  }
  ...
  if (aktimer30.isTimeOut()) {
    ...
    for (unsigned long i = 0; i < max_players2; i++) {
      ...
      if (player->isActive()) {
        ...
        /*
                        if (player->getPingLimitHits() >
           GameConfig::game_ping_limit && player->getDownLastPing() >
           GameConfig::game_ping_limit)
                        {
                            char chat_string_warning[140];
                            sprintf(chat_string_warning, "%s has a very slow
           connection (%.1f ms)!", player->getName().c_str(),
    ...
    for (unsigned long i = 0; i < max_players3; i++) {
      ...
      if (player->isActive()) {
        if (GameConfig::server_motd->length() > 0) {
          ...
          ChatInterface::serversayTo(player->getID(),
...
bool DedicatedGameManager::launchNetPanzerGame() {
  ...
  *Console::server << "contacting masterserver." << std::endl;
  if (GameConfig::server_public && !GameConfig::server_masterservers->empty()) {
    try {
      if (infosocket) {
        delete infosocket;
        infosocket = nullptr;
  ...
  *Console::server << "game started." << std::endl;
  ...
  if (GameConfig::server_interactive_console) {  // we may not always want the
                                                 // interactive console
...
//---------------------------------------------------------------------------
void DedicatedGameManager::initializeNetworkSubSystem() {
  BaseGameManager::initializeNetworkSubSystem();
}

void DedicatedGameManager::shutdownNetworkSubSystem() {
  if (infosocket) {
    delete infosocket;
    infosocket = nullptr;
  }

...
void PlayerGameManager::initializeSoundSubSystem() {
  ...
  if (sound == nullptr) sound = new DummySound();
...
void PlayerGameManager::initializeWindowSubSystem() {
  ...
  Desktop::checkResolution(iXY(800, 600),
                           iXY(screen->getWidth(), screen->getHeight()));
...
void PlayerGameManager::inputLoop() {
  ...
  Desktop::manage(MouseInterface::getMouseX(), MouseInterface::getMouseY(),
                  MouseInterface::getButtonMask());
...
void PlayerGameManager::hostMultiPlayerGame() {
  ...
  if (UnitProfileInterface::getNumUnitTypes() == 0) {
    LoadingView::append(
        "ERROR loading unit profiles, check your configuration");
    LoadingView::append("game.unit_profiles is: " +
                        *GameConfig::game_unit_profiles);
    graphicsLoop();
    SDL_Delay(5000);
    if (infosocket) {
      delete infosocket;
        ...
        if (i != PlayerInterface::getLocalPlayerIndex()) {
          ...
          if (player->isActive()) {
            if (player->checkAutokick()) {
              ...
              sprintf(chat_string, "Server kicked '%s' due to inactivity",

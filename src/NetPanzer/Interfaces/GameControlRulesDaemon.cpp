...
enum { _execution_mode_loop_back_server, _execution_mode_dedicated_server };
enum { _game_state_idle, _game_state_in_progress, _game_state_completed };
...
int GameControlRulesDaemon::map_cycle_fsm_server_state =
    _map_cycle_server_state_idle;
  ...
  switch (map_cycle_fsm_client_state) {
    ...
    case _map_cycle_client_start_map_load: {
      ...
      try {
        ...
        GameManager::startGameMapLoad(GameConfig::game_map->c_str(), nustyle,
                                      20);
      ...
      if (map_cycle_fsm_client_respawn_ack_flag == true) {
        ...
        if (NetworkState::status == _network_state_bot &&
          ...
          for (unsigned long i = 0; i < max_players; i++) {
            ...
            if (playerx->isActive()) {
              if (playerx->getID() != localplayer &&
                  playerx->getClientType() == 2) {
                PlayerAllianceRequest allie_request;
                allie_request.set(localplayer, playerx->getID(),
                                  _player_make_alliance);
                CLIENT->sendMessage(&allie_request,
...
void GameControlRulesDaemon::mapCycleFsmServer() {
  switch (map_cycle_fsm_server_state) {
    ...
    case _map_cycle_server_state_display_endgame_views: {
      ...
      view_control.set("EndRoundView", _view_control_flag_visible_on |
      ...
      map_cycle_fsm_server_endgame_timer.changePeriod(
          _MAP_CYCLE_ENDGAME_WAIT_PERIOD);
    ...
    case _map_cycle_server_state_cycle_next_map: {
      if (map_cycle_fsm_server_endgame_timer.count() &&
        ...
        if (nextmap != "") {
          ...
          GameConfig::game_map->assign(
              MapsManager::getNextMap(*GameConfig::game_map));
        }
        ...
        if (GameControlRulesDaemon::execution_mode ==
          ...
          GameManager::dedicatedLoadGameMap(GameConfig::game_map->c_str(),
                                            GameConfig::game_mapstyle->c_str());

          GameManager::resetGameLogicAlt();

          map_cycle_fsm_server_map_load_timer.changePeriod(
              _MAP_CYCLE_MAP_LOAD_WAIT_PERIOD);
          ...
          try {
            GameManager::startGameMapLoad(GameConfig::game_map->c_str(),
                                          GameConfig::game_mapstyle->c_str(),
                                          20);
          } catch (std::exception& e) {
            LoadingView::append("Error while loading map or mapstyle:");
    ...
    case _map_cycle_server_state_sync_profiles: {
      if (sync_profile_index <= UnitProfileInterface::getNumUnitTypes()) {
        Uint8 data[_MAX_NET_PACKET_SIZE];
        NetMessage* pmsg = (NetMessage*)&data;
        int len = UnitProfileInterface::fillProfileSyncMessage(
            pmsg, sync_profile_index);
        SERVER->broadcastMessage(pmsg, len);
        sync_profile_index++;
      }
      if (sync_profile_index >= UnitProfileInterface::getNumUnitTypes()) {
    ...
    case _map_cycle_server_state_respawn_players: {
      ...
      SERVER->broadcastMessage(&reset_game_logic_mesg,
                               sizeof(SystemResetGameLogic));
      ...
      SERVER->broadcastMessage(&respawn_ack_mesg,
                               sizeof(GameControlCycleRespawnAck));
  ...
  if ((GameControlRulesDaemon::game_state == _game_state_in_progress) &&
    ...
    switch (game_type) {
      ...
      case _gametype_fraglimit: {
        if (PlayerInterface::testRuleScoreLimit(GameConfig::game_fraglimit,
                                                &player_state) == true) {
      ...
      case _gametype_objectiveANDfraglimit: {
        float ratio = (float)GameConfig::game_occupationpercentage / 100.0;
        if ((PlayerInterface::testRuleObjectiveRatio(ratio, &player_state)) &&
            (PlayerInterface::testRuleScoreLimit(GameConfig::game_fraglimit,
                                                 &player_state) == true)) {
...
void GameControlRulesDaemon::processNetMessage(const NetMessage* message,
  ...
  switch (message->message_id) {
    ...
    default:
      LOGGER.warning("Received GameControlRulesMessage with unknown id (%d-%d)",
...
void GameControlRulesDaemon::mapLoadFailureResponse(int result_code,
                                                    const char* map_name) {
  char str_buf[128];

...
void MouseInterface::onMouseButtonDown(SDL_MouseButtonEvent* e) {
  if (!clickcount) {
    clicktimer.reset();
  }
...
void MouseInterface::onMouseButtonUp(SDL_MouseButtonEvent* e) {
  if (clickcount) {
    releasecount++;
  }
...
void MouseInterface::manageClickTimer() {
  if (clickcount && clicktimer.isTimeOut()) {
    // LOGGER.warning("Mouse click count and release count = %d, %d",
    // clickcount, releasecount);
    clickcount = 0;
    releasecount = 0;
  }
}

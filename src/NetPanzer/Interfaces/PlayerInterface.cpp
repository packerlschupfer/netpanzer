...
static void disconnectedPlayerAllianceCleanup(PlayerID index) {
  for (PlayerID player_index = 0;
       player_index < PlayerInterface::getMaxPlayers(); ++player_index) {
...
static void handleAllianceMessage(const int type, PlayerID by_player,
  ...
  if (type == _player_make_alliance) {
    ...
    if (by_player == local_player) {
      ...
      if (PlayerInterface::isSingleAllied(player_state->getID(),
                                          local_player)) {
      ...
      if (PlayerInterface::isSingleAllied(local_player,
                                          player_state->getID())) {
        ConsoleInterface::postMessage(Color::yellow, false, 0,
        ...
        if (player_state->getMute() == false) {
          ...
          if (NetworkState::status == _network_state_bot &&
              GameConfig::bot_allied == true &&
              GameConfig::game_allowallies == true) {
            if (player_state->getClientType() == 2) {
              PlayerAllianceRequest allie_request_back;
              allie_request_back.set(local_player, player_state->getID(),
                                     _player_make_alliance);
              CLIENT->sendMessage(&allie_request_back,
                                  sizeof(PlayerAllianceRequest));
            }
          }
          //
        }
...
static void resetAllianceMatrix() {
  ...
  matrix_size =
      PlayerInterface::getMaxPlayers() * PlayerInterface::getMaxPlayers();
...
bool PlayerInterface::testRuleScoreLimit(long score_limit,
                                         PlayerState** player_state) {
  PlayerID player_id;
...
static bool testRuleObjectiveOccupationRatio(PlayerID player_id,
  ...
  size_t occupation_ratio =
      (size_t)(((float)num_objectives) * precentage + 0.999);
...
bool PlayerInterface::testRuleObjectiveRatio(float precentage,
                                             PlayerState** player_state) {
  PlayerID player_id;
...
bool PlayerInterface::testRulePlayerRespawn(bool* completed,
                                            PlayerState** player_state) {
...
void PlayerInterface::netMessageConnectID(const NetMessage* message) {
  ...
  {
    const PlayerConnectID* connect_mesg = (const PlayerConnectID*)message;
...
void PlayerInterface::netMessageSyncState(const NetMessage* message) {
  if (NetworkState::status !=
      _network_state_server)  // client or bot only (security fix)
  {
    const PlayerStateSync* sync_mesg = (const PlayerStateSync*)message;
    ...
    if (player_id == local_player_index &&
        player_lists[player_id].isSelectingFlag()) {
...
void PlayerInterface::netMessageScoreUpdate(const NetMessage* message) {
  if (NetworkState::status == _network_state_client ||
      NetworkState::status == _network_state_bot)  // client only (security fix)
  {
    const PlayerScoreUpdate* score_update = (const PlayerScoreUpdate*)message;
...
void PlayerInterface::netMessageAllianceRequest(const NetMessage* message) {
  ...
  if (allie_request->getAllieByPlayerIndex() != local_player_index) {
    SERVER->sendMessage(allie_request->getAllieByPlayerIndex(), &allie_update,
                        sizeof(PlayerAllianceUpdate));
  }
  if (allie_request->getAllieWithPlayerIndex() != local_player_index) {
    SERVER->sendMessage(allie_request->getAllieWithPlayerIndex(), &allie_update,
                        sizeof(PlayerAllianceUpdate));
  }
}
...
void PlayerInterface::netMessageStyleSync(const NetMessage* message) {
  if (NetworkState::status == _network_state_client ||
      NetworkState::status == _network_state_bot) {
...
void PlayerInterface::processNetMessage(const NetPacket* packet) {
  ...
  switch (message->message_id) {
    ...
    case _net_message_id_player_sync_flag: {
      ...
      ResourceManager::updateFlagData(pfs->player_id, pfs->player_flag,
    ...
    case _net_message_id_player_update_flag: {
      ...
      ResourceManager::updateFlagData(packet->fromPlayer, upf->player_flag,
...
void PlayerInterface::SyncFlagTimer() {
  PlayerFlagTimerUpdate player_flagtimer_update(
      gameconfig->game_changeflagtime);

  SERVER->broadcastMessage(&player_flagtimer_update,
                           sizeof(PlayerFlagTimerUpdate));
}

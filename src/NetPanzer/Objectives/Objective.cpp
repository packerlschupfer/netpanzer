...
void Objective::changeOwner(PlayerState* new_owner) {
  if (occupying_player) {
  ...
  if (occupying_player) {
    ...
    ConsoleInterface::postMessage(Color::cyan, false, 0,
                                  "'%s' has been occupied by '%s'", name,
                                  new_owner->getName().c_str());
  }
...
void Objective::changeUnitGeneration(bool is_on, int unit_type) {
...
void Objective::syncFromData(const ObjectiveSyncData& sync_data) {
...
void Objective::attemptOccupationChange(PlayerState* player) {
  if (!player->isActive()) {
    return;
  }

  if (GameConfig::game_base_limit > 0 &&
      player->getObjectivesHeld() >= GameConfig::game_base_limit) {
    /*
            char base_limit_warning[140];
            sprintf(base_limit_warning, "Warning '%s' - Max %i bases per
       player!", player->getName().c_str(), GameConfig::game_base_limit);
...
void Objective::checkOccupationStatus() {
...
void Objective::generateUnits() {  // XXX CHECK
  if (occupying_player && unit_generation_on_flag == true) {
    if (unit_generation_timer.count() == true) {
      ...
      unit = UnitInterface::createUnit(unit_generation_type, gen_loc,
                                       occupying_player->getID());

      if (unit != 0) {
        UnitRemoteCreate create_mesg(unit->player->getID(), unit->id, gen_loc.x,

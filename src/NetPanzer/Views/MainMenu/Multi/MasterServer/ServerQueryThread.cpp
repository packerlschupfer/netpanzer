...
ServerQueryThread::ServerQueryThread(ServerList *newserverlist)
  ...
  try {
    ...
    running = false;
    if (udpsocket) udpsocket->destroy();
  }
}
...
ServerQueryThread::~ServerQueryThread() {
  ...
  if (!querying_msdata.empty()) {
    ...
    for (msiter = querying_msdata.begin(); msiter != querying_msdata.end();
         msiter++) {
...
void ServerQueryThread::queryMasterServer() {
  while (!masterservers.empty()) {
    try {
      ...
      network::TCPSocket *s =
          new network::TCPSocket(masterserverip, "28900", this);
...
void ServerQueryThread::onConnected(network::TCPSocket *s) {
  LOGGER.info("MASTERSERVER Connected [%s], sending query.",
              s->getAddress().getIP().c_str());
  //    char query[] =
  //    "\\list\\gamename\\master\\final\\list\\gamename\\netpanzer\\final\\";
...
void ServerQueryThread::onDisconnected(network::TCPSocket *s) {
  LOGGER.warning("MASTERSERVER Disconnected [%s]",
                 s->getAddress().getIP().c_str());
  delete querying_msdata[s];
  querying_msdata.erase(s);
}

void ServerQueryThread::onSocketError(network::TCPSocket *s) {
  LOGGER.warning("MASTERSERVER Socket error [%s]",
                 s->getAddress().getIP().c_str());
  delete querying_msdata[s];
  querying_msdata.erase(s);
}

void ServerQueryThread::onSocketError(network::UDPSocket *s) {
  LOGGER.warning("SERVER query socket error [%s]",
                 s->getAddress().getIP().c_str());
  udpsocket = nullptr;
}

void ServerQueryThread::onDataReceived(network::TCPSocket *s, const char *data,
                                       const int len) {
  LOGGER.debug("ServerQueryThread: Received [%s] from server [%s]", data,
               s->getAddress().getIP().c_str());
  ...
  while (!token.empty()) {
    if (token == "ip") {
      ...
      LOGGER.warning("Server IP received: [%s:%s]", dirip.c_str(),
                     port.c_str());
...
void ServerQueryThread::sendNextQuery() {
...
void ServerQueryThread::sendQuery(ServerInfo *server) {
  ...
  if (server->ipaddress == network::Address::ANY) {
    server->ipaddress =
        network::Address::resolve(server->address, server->port);
  }
...
void ServerQueryThread::onDataReceived(network::UDPSocket *s,
                                       const network::Address &from,
  ...
  if (server) {
    LOGGER.debug("Got data [%s] from server [%s]", str.c_str(),
                 fromaddress.str().c_str());
    parseServerData(server, str);
    querying_server.erase(fromaddress.str());
  } else {
    LOGGER.warning("Received answer from unknown server [%s]",
                   fromaddress.str().c_str());
    querying_server.erase(
        fromaddress.str());  // Quick & Dirty, should use find()
  }

  sendNextQuery();
}

void ServerQueryThread::parseServerData(ServerInfo *server, std::string &data) {
  ...
  while ((token = tokenizer.getNextToken()) != "") {
    if (token == "hostname") {
      ...
      server->auth_on =
          tokenizer.getNextToken().compare("y") == 0 ? true : false;
    } else if (token == "password") {
      server->needs_password =
          tokenizer.getNextToken().compare("y") == 0 ? true : false;
    } else {
      // handle more tokens...
    }
...
void ServerQueryThread::checkTimeOuts() {
  ...
  while (msiter != querying_msdata.end()) {
    if (now - msiter->second->lastTicks > MS_TIMEOUT) {
      LOGGER.warning("Masterserver [%s] timeout",
...
const char *ServerQueryThread::getStateMessage() const {
  switch (state) {

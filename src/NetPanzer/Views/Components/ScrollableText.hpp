...
public:
  ScrollableText(View *view, std::string input_text, iRect input_rect, const PIX input_color,
                 const PIX input_blend_color) : Component(), offsetY(0) {
    position.zero();
    text = input_text;
    rect = input_rect;
    color = input_color;
...
protected:
  ...
  PIX blend_color;
  int8_t direction;
  // 64bit just encase someone leaves the game open for months :)
  uint64_t scrollStartTime;
...
public:
  ...
  virtual void render() {};
  void setScrolling(bool isScrolling, int8_t newDirection);
  void scroll();
};

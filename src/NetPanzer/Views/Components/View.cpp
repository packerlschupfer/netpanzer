...
// View
//---------------------------------------------------------------------------
View::View() : currentscreen(0) { reset(); }  // end View::View
...
View::View(const iXY &pos, const iXY &size, const char *title) {
  ...
  // addButton(iXY(getSizeX()-10, getSizeY()-10), "pics/buttons/wclose.raw",
  // "Close the current window", View::toggleWindow); LOG(("Past addButton"));
...
void View::reset() {
  ...
  moveAreaHeight =
      Surface::getFontHeight();  // one row of text plus some padding
...
void View::drawTitle(Surface &viewArea) {
  ...
  iRect r(borderSize, borderSize, getSizeX() - borderSize,
          borderSize + moveAreaHeight - 1);
  ...
  if (getResize()) {
    {
      ...
      iXY horizontalPos(borderSize + moveAreaHeight - 1,
                        getSizeY() - borderSize);
    ...
    // Top-Right resize.
    {
      ...
      iXY horizontalPos(getSizeX() - borderSize - moveAreaHeight,
                        borderSize - 1);
    ...
    // Bottom-Right resize.
    {
      iXY verticalPos(getSizeX() - borderSize,
                      getSizeY() - borderSize - moveAreaHeight + 1);
      iXY horizontalPos(getSizeX() - borderSize - moveAreaHeight + 1,
                        getSizeY() - borderSize);
...
void View::draw(Surface &surface) {
  ...
  // LOGGER.info("Name: %s, active=%d, visible=%d, status=%d, resize=%d",
  // getSearchName(), getActive(), getVisible(), getShowStatus(), getResize());
...
void View::doActivate() {
  ...
  for (i = components.begin(); i != components.end(); i++) {
    if ((*i)->contains(MouseInterface::getMousePosition())) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_ENTERED, now(), 0,
                     MouseInterface::getMouseX(), MouseInterface::getMouseY(),
                     0, false);
...
void View::doDeactivate() {
  ComponentsIterator i;
  for (i = components.begin(); i != components.end(); i++) {
    mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_EXITED, now(), 0,
                   MouseInterface::getMouseX(), MouseInterface::getMouseY(), 0,
                   false);
...
int View::getMouseActions(const iXY &pos) const {
  ...
  if (getResize()) {
    ...
    minOff.x = abs(pos.x);
    maxOff.x = abs(pos.x - getSizeX());
    minOff.y = abs(pos.y);
    maxOff.y = abs(pos.y - getSizeY());

    // Check the left,  then the right
    if (minOff.x < RESIZE_XMIN)
      actions |= MA_RESIZE_LEFT;
    else if (maxOff.x < RESIZE_XMIN)
      actions |= MA_RESIZE_RIGHT;

    // Check the top, then the bottom
    if (minOff.y < RESIZE_YMIN)
      actions |= MA_RESIZE_TOP;
    else if (maxOff.y < RESIZE_YMIN)
      actions |= MA_RESIZE_BOTTOM;

    // If a left or right was turned on, then give more area for selecting the
    // resize area
    if (actions & (MA_RESIZE_LEFT | MA_RESIZE_RIGHT)) {
      if (minOff.y < RESIZE_YMAX)
        actions |= MA_RESIZE_TOP;
      else if (maxOff.y < RESIZE_YMAX)
        actions |= MA_RESIZE_BOTTOM;
    }

    if (actions & (MA_RESIZE_TOP | MA_RESIZE_BOTTOM)) {
      if (minOff.x < RESIZE_XMAX)
        actions |= MA_RESIZE_LEFT;
      else if (maxOff.x < RESIZE_XMAX)
        actions |= MA_RESIZE_RIGHT;
    }
  }
  ...
  if (getAllowMove()) {
    // iRect titleBar(borderSize, borderSize, getSizeX()-borderSize, borderSize
    // + moveAreaHeight);
    iRect titleBar(borderSize, borderSize, getSizeX() - borderSize,
  ...
  //{
  //	//iRect titleBar(borderSize, borderSize, getSizeX()-borderSize,
  // borderSize + moveAreaHeight); 	iRect titleBar(borderSize, getSizeY() -
  // borderSize - DEFAULT_STATUS_BAR_HEIGHT, getSizeX() - borderSize, getSizeY()
  //- borderSize); 	if (titleBar.contains(pos)) return actions |= MA_MOVE;
  //}
...
Surface *View::getClientArea(Surface &dest) {
  ...
  if (getBordered()) {
    iRect rect(borderSize, borderSize + moveAreaHeight, getSizeX() - borderSize,
               getSizeY() - borderSize);
...
iRect View::getClientRect() const {
  if (getBordered()) {
    return iRect(borderSize, borderSize + moveAreaHeight,
...
void View::mouseMove(const iXY &prevPos, const iXY &newPos) {
  ...
  for (i = components.begin(); i != components.end(); i++) {
    ...
    if (containsPrev && !containsNew) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_EXITED, now(), 0, newPos.x,
                     newPos.y, 0, false);
      ...
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_ENTERED, now(), 0, newPos.x,
                     newPos.y, 0, false);
      ...
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_MOVED, now(), 0, newPos.x,
                     newPos.y, 0, false);
...
void View::lMouseDown(const iXY &pos) {
  ...
  for (i = components.begin(); i != components.end(); i++) {
    if ((*i)->contains(pos)) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_PRESSED, now(),
                     InputEvent::BUTTON1_MASK, pos.x, pos.y, 0, false);
...
int View::lMouseUp(const iXY &downPos, const iXY &upPos) {
  ...
  if (pressedButton == findButtonContaining(upPos)) {
    if (pressedButton >= 0) {
      ...
      else if (buttons[pressedButton]->rightClickFunc != 0)
  ...
  {
    ComponentsIterator i;
    for (i = components.begin(); !components.empty() && i != components.end();
         i++) {
      if ((*i)->contains(downPos) && (*i)->contains(upPos)) {
        mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_CLICKED, now(),
                       InputEvent::BUTTON1_MASK, upPos.x, upPos.y, 0, false);
  ...
  // Report a mouse release to all components except for a clicked one.
  {
    ...
    for (i = components.begin(); !components.empty() && i != components.end();
         i++) {
      if ((*i)->contains(upPos)) {
        mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_RELEASED, now(),
                       InputEvent::BUTTON1_MASK, upPos.x, upPos.y, 0, false);
...
// lMouseDrag
//---------------------------------------------------------------------------
void View::lMouseDrag(const iXY &downPos, const iXY &prevPos,
                      const iXY &newPos) {
  // lMouseDown(newPos);
  ...
  for (i = components.begin(); !components.empty() && i != components.end();
       i++) {
    if ((*i)->contains(newPos) && (*i)->contains(downPos) &&
        newPos != prevPos) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_DRAGGED, now(),
                     InputEvent::BUTTON1_MASK, newPos.x, newPos.y, 0, false);
...
void View::rMouseDown(const iXY &pos) {
  ...
  for (i = components.begin(); !components.empty() && i != components.end();
    ...
    if ((*i)->contains(pos)) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_PRESSED, now(),
                     InputEvent::BUTTON2_MASK, pos.x, pos.y, 0, false);
...
void View::rMouseUp(const iXY &downPos, const iXY &upPos) {
  ...
  for (i = components.begin(); !components.empty() && i != components.end();
    ...
    if ((*i)->contains(downPos) && (*i)->contains(upPos)) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_CLICKED, now(),
                     InputEvent::BUTTON2_MASK, upPos.x, upPos.y, 0, false);
...
// rMouseUp
//---------------------------------------------------------------------------
void View::rMouseUp() {}  // end View::rMouseUp

// rMouseDrag
//---------------------------------------------------------------------------
void View::rMouseDrag(const iXY &downPos, const iXY &prevPos,
                      const iXY &newPos) {
  rMouseDown(newPos);
  ...
  for (i = components.begin(); i != components.end(); i++) {
    if ((*i)->contains(newPos) && (*i)->contains(downPos) &&
        newPos != prevPos) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_DRAGGED, now(),
                     InputEvent::BUTTON2_MASK, newPos.x, newPos.y, 0, false);
...
void View::mouseEnter(const iXY &pos) {
  ...
  for (i = components.begin(); i != components.end(); i++) {
    if ((*i)->contains(pos)) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_ENTERED, now(), 0, pos.x,
                     pos.y, 0, false);
...
void View::mouseExit(const iXY &pos) {
  ...
  for (i = components.begin(); i != components.end(); i++) {
    if ((*i)->contains(pos)) {
      mMouseEvent me((*i), mMouseEvent::MOUSE_EVENT_EXITED, now(), 0, pos.x,
                     pos.y, 0, false);
...
void View::drawHighlightedButton(Surface &clientArea) {
  ...
  if (buttons[highlightedButton]->topSurface.getFrameCount() < 2) {
    ...
    clientArea.drawRect(
        iRect(button->getBounds().min.x, button->getBounds().min.y,
              button->getBounds().max.x, button->getBounds().max.y),
        Color::red);
    return;
  }
  ...
  buttons[highlightedButton]->topSurface.blt(
      clientArea, iXY(buttons[highlightedButton]->getBounds().min.x,
...
//---------------------------------------------------------------------------
void View::addButtonCenterText(const iXY &pos, const int &xSize,
...
void View::drawStatus(Surface &dest) {
  ...
  iRect r(0, clientRect.getSizeY() - DEFAULT_STATUS_BAR_HEIGHT,
          clientRect.getSizeX(), clientRect.getSizeY());
...
void View::drawPressedButton(Surface &clientArea) {
  ...
  if (pressedButton < 0 ||
      buttons[pressedButton]->topSurface.getFrameCount() < 2)
    return;
  ...
  buttons[pressedButton]->topSurface.blt(
      clientArea, iXY(buttons[pressedButton]->getBounds().min.x,
...
void View::checkArea(iXY viewarea) {
  if (min.x < 0) moveTo(0, min.y);
  if (min.y < 0) moveTo(min.x, 0);

  if (max.x > viewarea.x) moveTo(viewarea.x - getSize().x, min.y);
  if (max.y > viewarea.y) moveTo(min.x, viewarea.y - getSize().y);
}
...
void View::setDisplayStatusBar(const bool &newStatus) {
  if (newStatus == true)
    status |= STATUS_DISPLAY_STATUS_BAR;
  else
    status &= ~STATUS_DISPLAY_STATUS_BAR;
}  // setDisplayStatusBar
...
void View::setAlwaysOnBottom(const bool &newStatus) {
  if (newStatus == true)
    status |= STATUS_ALWAYS_ON_BOTTOM;
  else
    status &= ~STATUS_ALWAYS_ON_BOTTOM;
}  // setAlwaysOnBottom
...
// addInputField
//---------------------------------------------------------------------------
cInputField *View::addInputField(const iXY &pos, cInputFieldString *string,
...
void View::removeInputField(cInputField *cif) {
  if (cif) {
    inputFields.erase(std::remove(inputFields.begin(), inputFields.end(), cif),
                      inputFields.end());
    delete cif;
  }
}

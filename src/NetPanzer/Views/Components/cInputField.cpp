...
cInputField::~cInputField() { delete[] excludedCharacters; }
...
void cInputField::addChar(int newChar) {
  ...
  if (textaction != 0) textaction(this);
}  // addChar

void cInputField::setTextAction(ACTION_FUNC_PTR action) { textaction = action; }
...
void cInputField::addExtendedChar(int newExtendedChar) {
  ...
  switch (newExtendedChar) {
    ...
    case SDLK_DELETE: {
      ...
      memmove(destString + cursorPos, destString + cursorPos + 1,
              strlen(destString + cursorPos + 1) + 1);
    } break;
...
void cInputField::drawHighlighted(Surface &dest) {
  ...
  if ((timeForBlink += TimerInterface::getTimeSlice()) > 0.25f) {
    ...
    int cursorPos = std::max(
        std::min(cInputField::cursorPos - strDisplayStart, maxCharCount - 1),
        (size_t)0);
    int charWidth = Surface::getTextWidth(" ");

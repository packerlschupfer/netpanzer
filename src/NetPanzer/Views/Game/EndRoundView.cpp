...
#define WINDOW_WIDTH \
  (TABLE_HEADER_PIX_LEN + ((DEFAULT_BORDER_SIZE + TABLE_BORDER) * 2) + 14 + 2)
...
class StatesSortByGoals {
 public:
  bool operator()(const PlayerState* state1, const PlayerState* state2) {
    ...
    bool b1 = ((state1->getTotal() >= GameConfig::game_fraglimit) &&
               (state1->getObjectivesHeld() >= ratio));
    bool b2 = ((state2->getTotal() >= GameConfig::game_fraglimit) &&
               (state2->getObjectivesHeld() >= ratio));

    return b1 > b2 || (b1 == b2 && p1 > p2) ||
           (b1 = b2 && p1 == p2 && state1->getTotal() > state2->getTotal());
  }
};
// drawPlayerStats
//---------------------------------------------------------------------------
...
void EndRoundView::drawPlayerStats(Surface& dest, unsigned int flagHeight, PIX winnerBlendColor, PIX tableHeaderBlendColor, PIX nextGameBlendColor) {
  ...
  for (std::vector<const PlayerState*>::iterator i = states.begin();
    ...
    colorImage.bltTransColor(dest, TABLE_HEADER_PIX_LEN + 5, flag_pos,
                             state->getColor());
...
void EndRoundView::doActivate() { selected_line = -1; }

void EndRoundView::doDeactivate() { selected_line = -1; }

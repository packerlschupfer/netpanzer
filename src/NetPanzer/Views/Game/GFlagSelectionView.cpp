...
class GFlagButton : public Button {
 public:
  GFlagButton(int x, int y, Surface &s) : Button("gflag") {
    setLocation(x, y);
  ...
  void actionPerformed(const mMouseEvent &e) {
    if (e.getID() == mMouseEvent::MOUSE_EVENT_CLICKED) {
      ...
      memcpy(upf.player_flag, GameConfig::player_flag_data,
             sizeof(GameConfig::player_flag_data));
...
void GFlagSelectionView::init() {
  ...
  // add( new Label(tx, ty, "Current:", windowTextColor, windowTextColorShadow,
  // true) );
...
void GFlagSelectionView::doDraw(Surface &viewArea, Surface &clientArea) {
  ...
  ResourceManager::getFlag(PlayerInterface::getLocalPlayerIndex())
      ->blt(clientArea, loc_player_flag.x, loc_player_flag.y);
...
void GFlagSelectionView::checkResolution(iXY oldResolution, iXY newResolution) {
  resize(iXY(newResolution.x, newResolution.y));
  moveTo(iXY(0, 0));
}

void GFlagSelectionView::processEvents() { COMMAND_PROCESSOR.process(false); }

...
static void notExistsError(lua_State *L) {
  ...
  lua_getstack(L, 1, &ar);
  lua_getinfo(L, "Sl", &ar);
  LOGGER.warning("ERROR in '%s':%d: config '%s' is invalid.", ar.source,
                 ar.currentline, lua_tostring(L, 2));
}
...
static void notModifiedError(lua_State *L, const char *what) {
  ...
  LOGGER.warning("ERROR in '%s':%d: '%s' not modified, value is not a %s.",
                 ar.source, ar.currentline, lua_tostring(L, 2), what);
}
...
int ScriptHelper::set_int(lua_State *L, void *v) {
...
int ScriptHelper::get_number(lua_State *L, void *v) {
  lua_pushnumber(L, *(lua_Number *)v);
  return 1;
}

int ScriptHelper::set_number(lua_State *L, void *v) {
...
int ScriptHelper::get_string(lua_State *L, void *v) {
...
int ScriptHelper::set_string(lua_State *L, void *v) {
...
int ScriptHelper::get_boolean(lua_State *L, void *v) {
  lua_pushboolean(L, *(bool *)v);
  return 1;
}

int ScriptHelper::set_boolean(lua_State *L, void *v) {
  *(bool *)v = lua_toboolean(L, 3);
  return 0;
}

int ScriptHelper::staticVarCall(lua_State *L) {
  /* for get: stack has userdata, index, lightuserdata */
  /* for set: stack has userdata, index, value, lightuserdata */
  ScriptVarBindRecord *m =
      (ScriptVarBindRecord *)lua_touserdata(L, -1); /* member info */
  lua_pop(L, 1);                                    /* drop lightuserdata */
  return m->func(L, (void *)(m->data));
}

int ScriptHelper::index_handler(lua_State *L) {
...
int ScriptHelper::newindex_handler(lua_State *L) {
...
int ScriptHelper::next_handler(lua_State *L) {
  /* stack has table, key */
  ScriptVarBindRecord *record =
      (ScriptVarBindRecord *)lua_topointer(L, lua_upvalueindex(1));
  if (!lua_isnil(L, 2)) {
    // find this key
...
int ScriptHelper::autotable_indexhandler(lua_State *L) {

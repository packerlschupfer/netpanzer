...
enum { _powerup_unit, _powerup_bonus_units, _powerup_global_unit };

int powerup_probability_table[3] = {_powerup_unit, _powerup_bonus_units,
...
PowerUpID PowerUpInterface::getNextPowerUpID() {
...
void PowerUpInterface::generatePowerUp() {
  ...
  while (powerup_list.size() < (size_t)power_up_limit) {
    ...
    // LOGGER.info("broadcasting  plsize=%d id=%d type=%d", powerup_list.size(),
    // power_up->ID, power_up->type);
    SERVER->broadcastMessage(&create_mesg, sizeof(create_mesg));

    int regrange =
        power_up_regen_time_upper_bound - power_up_regen_time_lower_bound;
...
void PowerUpInterface::updateState() {
  ...
  for (PowerUpList::iterator i = powerup_list.begin(); i != powerup_list.end();
       /* empty */) {
    PowerUp* powerup = *i;
...
void PowerUpInterface::netMessagePowerUpCreate(const NetMessage* message) {
  ...
  // LOGGER.info("Got 1 Pwp Create msg!!! size is %d id=%d type=%d x=%d, y=%d",
  // powerup_list.size(), create_mesg->getID(), create_mesg->getType(),
  // create_mesg->getLocX(), create_mesg->getLocY());
  ...
  switch (create_mesg->getType()) {
    ...
    case _powerup_unit:
      power_up = new UnitPowerUp(
          iXY(create_mesg->getLocX(), create_mesg->getLocY()), _powerup_unit);
      break;
...
void PowerUpInterface::syncPowerUps(ClientSocket* client) {
  ...
  for (i = powerup_list.begin(); i != powerup_list.end(); i++) {
    ...
    PowerUpCreateMesg create_mesg;
    create_mesg.set(powerup_ptr->map_loc, powerup_ptr->ID, powerup_ptr->type);

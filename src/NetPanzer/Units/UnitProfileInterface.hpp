...
class UnitProfileInterface {
 ...
 public:
  ...
  static unsigned int getNumUnitTypes() { return profiles.size(); }
  ...
  static UnitProfile* loadProfileFromMessage(const NetMessage* message,
                                             size_t size);
  static void processNetMessage(const NetMessage* net_message, size_t size);
  static void handleProfileDescMessage(const NetMessage* net_message,
                                       size_t size);
};

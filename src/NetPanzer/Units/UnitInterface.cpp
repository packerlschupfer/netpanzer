...
void UnitInterface::processNetPacket(const NetPacket* packet) {
...
void UnitInterface::sendMessage(const UnitMessage* message,
  ...
  if (message->isFlagged(_umesg_flag_unique)) {
    ...
    if (player) {
      LOGGER.warning("UnitManagerMessage sent out by player %u not allowed.",
                     player->getID());
      return;
    }
...
void UnitInterface::updateUnitStatus() {
  for (Units::iterator i = units.begin(); i != units.end(); /*nothing*/) {
    ...
    pre_update_bucket_index =
        unit_bucket_array.worldLocToBucketIndex(unit->unit_state.location);
    unit->updateState();
    ...
    if (post_update_bucket_index != pre_update_bucket_index) {
      unit_bucket_array.moveUnit(unit->id, pre_update_bucket_index,
                                 post_update_bucket_index);
    }
    ++i;
  }
...
void UnitInterface::offloadGraphics(SpriteSorter& sorter) {
  ...
  for (long row_index = bucket_rect.min.y; row_index <= bucket_rect.max.y;
       row_index++) {
    for (long column_index = bucket_rect.min.x;
      ...
      for (UnitBucketPointer* t = bucket_list->getFront(); t != 0;
           t = t->next) {
...
UnitID UnitInterface::newUnitID() {
  ...
  while (getUnit(newID) != 0) newID = lastUnitID++;
...
UnitBase* UnitInterface::newUnit(unsigned short unit_type, const iXY& location,
                                 PlayerID player_index, UnitID id) {
  UnitBase* unit = 0;
  ...
  if (unit_type < UnitProfileInterface::getNumUnitTypes()) {
    ...
    bool not_live = false;
    unit = new Vehicle(not_live, player, unit_type, id, unit_style, moving,
                       location, body_angle_nl, turret_angle_nl, orientation_nl,
...
void UnitInterface::sortBucketArray() {
  ...
  for (bucket_index = 0; bucket_index < bucket_count; bucket_index++) {
    ...
    while (traversal_ptr != 0) {
      ...
      unit_bucket_index = unit_bucket_array.worldLocToBucketIndex(
          traversal_ptr->unit->unit_state.location);

      if (unit_bucket_index != bucket_index) {
        unit_bucket_array.moveUnit(traversal_ptr->unit->id, bucket_index,
                                   unit_bucket_index);
      }
...
UnitBase* UnitInterface::getUnit(UnitID id) {
...
UnitBase* UnitInterface::createUnit(unsigned short unit_type,
                                    const iXY& location, PlayerID player_id) {
  if (playerUnitLists[player_id].size() >= units_per_player) return 0;
...
void UnitInterface::spawnPlayerUnits(const iXY& location, PlayerID player_id,
  ...
  for (unit_type_index = 0;
       unit_type_index < UnitProfileInterface::getNumUnitTypes();
       unit_type_index++) {
    unsigned char real_uti = unit_type_index;
    unit_spawn_count = unit_config.getSpawnUnitCount(unit_type_index);
    for (unit_spawn_index = 0; unit_spawn_index < unit_spawn_count;
         unit_spawn_index++) {
      ...
      UnitRemoteCreate create_mesg(unit->player->getID(), unit->id, next_loc.x,
...
void UnitInterface::queryUnitsAt(std::vector<UnitID>& working_list,
                                 const iXY& point, PlayerID player_id,
                                 unsigned char search_flags) {
  for (Units::iterator i = units.begin(); i != units.end(); ++i) {
    UnitBase* unit = i->second;
    if (!unit->unit_state.bounds(point)) continue;

    if (search_flags == _search_exclude_player &&
        unit->player->getID() == player_id)
      continue;
    if (search_flags == _search_player && unit->player->getID() != player_id)
      continue;
...
void UnitInterface::queryUnitsAt(std::vector<UnitID>& working_list,
  ...
  for (Units::iterator i = units.begin(); i != units.end(); ++i) {
    UnitBase* unit = i->second;
    if (!rect.contains(unit->unit_state.location)) continue;
...
PlayerState* UnitInterface::querySinglePlayerInArea(const iRect& rect) {
...
bool UnitInterface::queryClosestUnit(UnitBase** closest_unit_ptr, iXY& loc,
                                     PlayerID player_id,
                                     unsigned char search_flags) {
...
bool UnitInterface::queryClosestEnemyUnit(UnitBase** closest_unit_ptr, iXY& loc,
...
void UnitInterface::unitManagerMesgEndLifecycle(const UnitMessage* message) {
  ...
  if (unit1 == 0 || unit2 == 0) {
    LOGGER.warning(
        ...
        "(u1: %u u2: %u)",
        lifecycle_update->getDestroyer(), lifecycle_update->getDestroyed());
    return;
  }
...
void UnitInterface::unitSyncMessage(const NetMessage* net_message) {
  ...
  try {
    std::map<UnitID, UnitBase*>::iterator uit =
        units.find(sync_message->getUnitID());
    if (uit != units.end()) {
      LOGGER.warning(
          "UnitInterface::unitSyncMessage() Received an existing unit [%d]",
          sync_message->getUnitID());
      return;
    }
    UnitBase* unit =
        newUnit(sync_message->unit_type,
                iXY(sync_message->getLocX(), sync_message->getLocY()),
                sync_message->getPlayerID(), sync_message->getUnitID());
    unit->in_sync_flag = false;
    addNewUnit(unit);
  } catch (std::exception& e) {
    LOGGER.warning("UnitInterface::unitSyncMessage() Couldn't sync unit '%s'",
                   e.what());
  }
}
...
void UnitInterface::unitOpcodeMessage(const NetMessage* net_message,
  ...
  while (decoder.decode(&opcode)) {
    ...
    if (!unit) {
      LOGGER.debug("Update for non-existant unit: %d", opcode->getUnitID());
      continue;
    }
...
void UnitInterface::unitCreateMessage(const NetMessage* net_message) {
  const UnitRemoteCreate* create_mesg = (const UnitRemoteCreate*)net_message;

  PlayerID player_index = create_mesg->getPlayerID();

  try {
    std::map<UnitID, UnitBase*>::iterator uit =
        units.find(create_mesg->getUnitID());
    if (uit != units.end()) {
      LOGGER.warning(
          "UnitInterface::unitCreateMessage() Received an existing unit [%d]",
          create_mesg->getUnitID());
      return;
    }
    iXY unitpos(create_mesg->getLocX(), create_mesg->getLocY());
    UnitBase* unit = newUnit(create_mesg->unit_type, unitpos, player_index,
                             create_mesg->getUnitID());
    addNewUnit(unit);
    ...
    LOGGER.warning(
        "UnitInterface::unitSyncMessage() Couldn't create new unit '%s'",
        e.what());
  }
}
...
void UnitInterface::unitModSpeedMessage(const NetMessage* net_message) {
  const UnitModSpeed* mod_speed_mesg = (const UnitModSpeed*)net_message;
...
void UnitInterface::unitModReloadMessage(const NetMessage* net_message) {
  const UnitModReload* mod_reload_mesg = (const UnitModReload*)net_message;
...
void UnitInterface::unitModFireMessage(const NetMessage* net_message) {
  const UnitModFire* mod_fire_mesg = (const UnitModFire*)net_message;
...
void UnitInterface::unitModWRangeMessage(const NetMessage* net_message) {
  const UnitModWRange* mod_w_range_mesg = (const UnitModWRange*)net_message;
...
void UnitInterface::unitModMHPMessage(const NetMessage* net_message) {
  const UnitModMHP* mod_mhp_mesg = (const UnitModMHP*)net_message;
...
void UnitInterface::unitModGRMessage(const NetMessage* net_message) {
  const UnitModGR* mod_gr_mesg = (const UnitModGR*)net_message;
  ...
  // loop here
  for (PlayerUnitList::iterator i = unitlist.begin(); i != unitlist.end();
       ++i) {
    UnitBase* unitx = *i;
...
void UnitInterface::unitModGSpeedMessage(const NetMessage* net_message) {
  const UnitModGS* mod_gs_mesg = (const UnitModGS*)net_message;
...
void UnitInterface::unitModGReloadMessage(const NetMessage* net_message) {
  const UnitModGRT* mod_grt_mesg = (const UnitModGRT*)net_message;
...
void UnitInterface::unitModGFireMessage(const NetMessage* net_message) {
  const UnitModGDM* mod_gdm_mesg = (const UnitModGDM*)net_message;
...
void UnitInterface::unitCreateMessageFull(const NetMessage* net_message) {
  ...
  try {
    std::map<UnitID, UnitBase*>::iterator uit =
        units.find(create_mesg_full->getUnitID());
    if (uit != units.end()) {
      LOGGER.warning(
          "UnitInterface::unitCreateMessage() Received an existing unit [%d]",
    ...
    if (unit_type < UnitProfileInterface::getNumUnitTypes()) {
      bool live = true;
      unit = new Vehicle(live, player, unit_type, id, unit_style, moving,

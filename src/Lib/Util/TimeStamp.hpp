...
class TimeStamp {
 private:
  ...
  inline static double ticksToSecs(Ticks t) { return double(t) / 1000.0; }
  inline static Ticks secsToTicks(float s) { return (Ticks)(s * 1000.0); }

 public:
  TimeStamp(Ticks nV = 0) { v = nV; }
  TimeStamp(const TimeStamp &other) { v = other.v; }
  ...
  double getSecs() const { return ticksToSecs(v); }

  double operator-(const TimeStamp &a) const { return ticksToSecs(v - a.v); }
  bool operator<(const TimeStamp &a) const { return v < a.v; }
  bool operator<=(const TimeStamp &a) const { return v <= a.v; }
  bool operator>=(const TimeStamp &a) const { return v >= a.v; }
  bool operator>(const TimeStamp &a) const { return v > a.v; }
  bool operator==(const TimeStamp &a) const { return v == a.v; }
  ...
  Uint32 getNetworkTicks() const { return v; }
};

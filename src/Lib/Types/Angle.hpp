...
class Angle {
 ...
 public:
  Angle(double nAngle = 0.0) { angle = nAngle; }

  Angle(long x, long y) { angle = (double)atan2((double)-y, (double)x); }

  Angle(const iXY& vec) { set(vec); }
...
class AngleInt {
 public:
  ...
  inline long operator++() {
    angle_int++;
    if (angle_int == angle_limit) angle_int = 0;
    return (angle_int);
  }

  inline long operator--() {
    angle_int--;
    if (angle_int < 0) angle_int = angle_limit - 1;
    return (angle_int);
  }

  inline long Degrees() const { return (angle_int * grain); }

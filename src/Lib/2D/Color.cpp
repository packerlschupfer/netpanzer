...
#define GEN_GETSTRUCT(CNAME) \
  {#CNAME, GETSVTYPE_BYTE, &MERGE_CLASS(Color) CNAME},
#define GEN_SETSTRUCT(CNAME) \
  {#CNAME, SETSVTYPE_BYTE, &MERGE_CLASS(Color) CNAME},
...
static const luaL_Reg color_methods[] = {{"rgb", color_rgb}, {0, 0}};

void Color::registerScript(const NPString& table_name) {
